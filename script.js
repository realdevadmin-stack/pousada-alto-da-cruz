import { enviarReserva, firebaseReady } from './firebase.js';
...
bookingForm.addEventListener('submit', async function (e) {
  ...
  try {
    ...
    formMessage.textContent = firebaseReady ? 'Erro ao enviar reserva. Tente novamente ou use o WhatsApp.' : 'Firebase não configurado. Use o botão Consultar no WhatsApp.';
    formMessage.classList.add('error');
...
whatsappBtn.addEventListener('click', function () {
  ...
  const pousadaWhatsApp = '5571985610497';

  if (nome && telefone && checkin && checkout && quarto) {
    const message = `Olá! Meu nome é ${nome}. Meu WhatsApp é ${telefone}. Gostaria de consultar reserva para ${quarto}, de ${checkin} até ${checkout}. Valor estimado: ${valor}.`;
    this.href = `https://wa.me/${pousadaWhatsApp}?text=${encodeURIComponent(message)}`;
  } else {
    this.href = `https://wa.me/${pousadaWhatsApp}`;
